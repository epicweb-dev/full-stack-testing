...
<EpicVideo url="https://www.epicweb.dev/workshops/web-application-testing/unit-testing/creating-a-setup-file-for-test-environment-setup" />
...
<callout-info>
	At the time of the original recording, our setup code also included
	`installGlobals` from `@remix-run/node`. This would set up things like
	`Headers`, `fetch`, etc. In modern Node.js, we no longer need these globals
	installed because they are now built in. The exercise content has been updated
